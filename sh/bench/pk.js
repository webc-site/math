...
export const run = async () => {
  ...
  const zh_path = join(ROOT, "readme/zh/README.md"),
    en_path = join(ROOT, "readme/en/README.md"),
    raw_kb = (oursSize.raw / 1024).toFixed(2),
    gz_kb = (oursSize.gz / 1024).toFixed(2);

  if (existsSync(zh_path)) {
    writeFileSync(
      zh_path,
      readFileSync(zh_path, "utf8").replace(
        /(- \*\*轻量化\*\*：核心包体积 )[\d.]+ KB（Gzip 压缩后 [\d.]+ KB）/,
        "$1" + raw_kb + " KB（Gzip 压缩后 " + gz_kb + " KB）",
      ),
    );
  }
  ...
  if (existsSync(en_path)) {
    writeFileSync(
      en_path,
      readFileSync(en_path, "utf8").replace(
        /(- \*\*Lightweight\*\*: Core package size is )[\d.]+ KB \([\d.]+ KB gzipped\)/,
        "$1" + raw_kb + " KB (" + gz_kb + " KB gzipped)",
      ),
    );
  }
